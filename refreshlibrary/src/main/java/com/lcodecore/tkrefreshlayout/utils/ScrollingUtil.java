...
public class ScrollingUtil {
    ...
    public static boolean isViewToBottom(View view, int mTouchSlop) {
        ...
        //这个为我们自己添加的判断NestedScrollView是否滑到底部的方法
        //那么向上加载也可以利用同样的方法来解决
        if (view instanceof TbNestedScrollView) return isTbestedScrollViewToBottom((TbNestedScrollView) view);
        if (view instanceof ViewGroup) return isViewGroupToBottom((ViewGroup) view);
        return false;
    }

    private static boolean isTbestedScrollViewToBottom(TbNestedScrollView view) {
